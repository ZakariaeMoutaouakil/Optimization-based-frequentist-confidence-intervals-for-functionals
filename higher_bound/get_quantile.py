...
from higher_bound.generate_tuple import generate_tuple
...
def get_quantile(alpha: float, q: float, n: int, m: int, indices: Tuple[Tuple[Tuple[int, ...]], ...]) -> int:
    ...
    quantile = multinomial_max_cdf_inverse(prob=alpha, n=n, p=p, indices=indices)
    return quantile
...
if __name__ == "__main__":
    ...
    lower_bounds = [get_quantile(alpha_, q, n_, m_, indices_) for q in q_values]
