...
def generate_indices(maximum: int, dimension: int) -> Tuple[Tuple[int, ...]]:
    """
    ...
    return tuple(ndindex(*(maximum + 1 for _ in range(dimension))))
...
if __name__ == "__main__":
    ...
    indices = generate_indices(maxi, dim)
    print(indices)
    assert len(indices) == (maxi + 1) ** dim
